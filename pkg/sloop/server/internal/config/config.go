...
type SloopConfig struct {
	...
	BadgerNumLevelZeroTables 		int           `json:"badgerNumLevelZeroTables"`
	BadgerNumLevelZeroTablesStall   int			  `json:"badgerNumLevelZeroTables"`
...
func registerFlags(fs *flag.FlagSet, config *SloopConfig) {
	...
	fs.IntVar(&config.BadgerNumLevelZeroTables, "badger-number-of-level-zero-tables", 0, "Number of level zero tables for badger")
	fs.IntVar(&config.BadgerNumLevelZeroTablesStall, "badger-number-of-zero-tables-stall", 0, "Number of Level 0 tables that once reached causes the DB to stall until compaction succeeds")
